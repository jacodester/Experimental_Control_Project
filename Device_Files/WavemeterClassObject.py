...
@ Slight modifications made so that things run for our needs and without 
...
class pyBristolSCPI:
    ...
    def readWL(self):
        """
        Retunrs float of the wavelength given in nm
    ...
    def readFREQ(self):
        """
        Same as above bu the frequency in THZ

        Returns
        -------
        float
            frequency output in THZ
    ...
    def readALL(self):
        """
        Returns a list of all the different parameter values
        position 0 is the frequency
        position 1 is ?
        position 2 is the wavelength
        position 3 is ?
    ...
    def readBuffer(self, outfile, acq_time):
        ...
        fs.close()
# These remaining methods are for a model wavemeter that we do not posses. 
'''        
