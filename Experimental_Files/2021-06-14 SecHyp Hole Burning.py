...
WFName1='JHD_Read_149_5MHz_151_5MHz_1000us'
startFreq1=149.5e6
sweepWidth1=1e6
...
freq2= wf.tanHypeSawtooth(2, 0.001e6, 150e6, len(t2),1)
wfmData2 = signal.sawtooth(2*np.pi*np.multiply(freq2,t2))
wfmData2 = wf.OptimiseWaveform(wfmData2,freq2,7)
MData1= np.ones(len(wfmData2))
MData2= np.ones(len(wfmData2))
...
Scope.WriteData('C2',"{}us_{}_{}V.txt".format(round(D2*1e6*reps,3),shape,high), freqs_out, 'MHz', voltages_out, vertical_unit_out, voltages_out, vertical_unit_out)
Scope.close()
...
Freq_new,OD=af.CalcODRegion(freqs_out,voltages_out1,10000,149.5,160)
...
Depth,Width1,Inds,minInd=af.CalcHoleParam(Freq_new, OD, 149.5, 150.5, 149.60, 149.80)
