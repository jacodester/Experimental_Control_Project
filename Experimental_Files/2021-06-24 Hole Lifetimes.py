...
'''
#Make and Upload Second Waveform: BURN
D2 = 200*1e-6 # specified waveform duration in sec
P2 = wf.Points(SR,D2)
t2=wf.Times(SR,duration=D2)
WFName2='NAD_Hole_SecHype_100us'
freq2= wf.tanHypeSawtooth(2, 0.000001e6, centerfreq, len(t2),1)
wfmData2 = signal.sawtooth(2*np.pi*np.multiply(freq2,t2))
wfmData2 = wf.OptimiseWaveform(wfmData2,freq2,7)
MData1= np.ones(len(wfmData2))
MData2= np.ones(len(wfmData2))
...
Depth,Width1,Inds,minInd=af.CalcHoleParam(Freq_new, OD, startFreq1*1e-6, startFreq1*1e-6+sweepWidth1*1e-6, 149.60, 149.80)
...
SampleTime=10 # sample time in seconds
...
for R in range(NumReads-1):
    ...
    #af.MakeBaseFig(2,Freq_new, OD, "Freq", "OD",Filename="C:/Users/LocalAdmin/Desktop/data/{}us_{}_{}V_OD.png".format(round(D2*1e6*reps,3),shape,high),close=1)
    Depth,Width1,Inds,minInd=af.CalcHoleParam(Freq_new, OD, startFreq1*1e-6, startFreq1*1e-6+sweepWidth1*1e-6, 149.6, 149.80)
    Xmin.append(Freq_new[minInd[0][0]])
