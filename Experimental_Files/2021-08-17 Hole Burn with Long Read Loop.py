...
WFName1='JHD_Read_149_5MHz_151_5MHz_2ms'
startFreq1=149e6
sweepWidth1=2e6
...
Scope.SetOffset('C2',-0.08,"VOLTS") #35mV/div
Scope.SetVDiv('C2',0.02)   #20mV/div
...
Scope.WriteData('C2',"{}us_{}_{}V_0.txt".format(round(D2*1e6*reps,3),0,high), freqs_out, 'MHz', voltages_out, vertical_unit_out, voltages_out, vertical_unit_out)
Scope.close()
...
Freq_new,OD=af.CalcODRegion(freqs_out,voltages_out1,10000,startFreq1*1e-6,startFreq1*1e-6+sweepWidth1*1e-6)
...
TotalTime=1 # total run time in minutes
...
for R in range(NumReads-1):
    ...
    Scope.close()
    voltages_out1=af.SavgolSmooth(voltages_out,51,3)
    af.MakeBaseFig(1,freqs_out, voltages_out, "Freq", "Voltage/Transmission",Filename="C:/Users/LocalAdmin/Desktop/data/{}us_{}_{}V.png".format(round(D2*1e6*reps,3),(R+1)*SampleTime+R+1,high))
    Freq_new,OD=af.CalcODRegion(freqs_out,voltages_out1,10000,startFreq1*1e-6,startFreq1*1e-6+sweepWidth1*1e-6)
