...
durations=np.arange(1e-4,1.1e-3,1e-4)
WFNames=[]

for step,dur in enumerate(durations):
    ...
    MData1[0:9]=0
    MData2[0:9]=0
    WFName5='JHD_Hole_149MHz_and_151MHz_2X{}us'.format(round(dur*1e6,2))
...
for step,waveform in enumerate(WFNames):
    ...
    reps=total/(durations[step]*2)
    # Make Sequence of the uploaded waveforms
    SEQList=[[(waveform,''), 1, 'BTR', round(reps), 0],
             [(WFName1,''), 2, 'BTR', 0, 0],
             [(WFName4,''), 3, 'OFF', 200, 1]
             ]
    AWG1=rm.open_resource("TCPIP0::1.1.1.5::INSTR",resource_pyclass=TO.AWG)
...
#-----------------------------------------------------------------------------
    ...
    FileName="{}usEach_2_Holes_0G".format(round(durations[step]*1e6,2))
    Scope=rm.open_resource("TCPIP0::1.1.1.4::INSTR",resource_pyclass=SO.LecroyScope)
    Scope.timeout=200000
    PulseBlaster1.Start()
    time.sleep(1)
