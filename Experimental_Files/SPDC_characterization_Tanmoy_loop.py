...
for step,value in enumerate(startFreq2):
    #Make and Upload Waveform: TRENCH
    
    P2 = wf.Points(SR,D2)
    t2=wf.Times(SR,duration=D2)
    WFName2='AJ_Trench_{}MHz_{}BW_MHz_2ms'.format(startFreq2[step],startFreq2[step]+2)
    
    freq2=wf.chirpedSawtooth(startFreq2[step]*1e6,sweepWidth2,len(t2))
...
#      ]
    SEQList=[[(WFName2,''), 1, 'BTR', reps, 0],[(WFName1,''), 2, 'BTR', 0, 0],[(WFName3,''), 3, 'OFF', 2, 1]]
    AWG1=rm.open_resource("TCPIP0::1.1.1.5::INSTR",resource_pyclass=TO.AWG)
    ...
    #-----------------------------------------------------------------------------
    PulseBlaster1.ResetProg()
    PulseBlaster1.WriteCommand([0,1], 0, 1) # 1us Trigger for AFG and AWG and Loop
    PulseBlaster1.WriteCommand([], 0, D2*reps*1e6) # Do nothing for duration of Burn
    PulseBlaster1.WriteCommand([], 0, waiting_time)  # waiting in us and end the loop
    PulseBlaster1.WriteCommand([0,2], 0, D1*1e6) #Read and trigger the AWG and Scope
    PulseBlaster1.WriteCommand([], 1, 5000) # Recycle and stop   #change this to 6
    PulseBlaster1.ViewProg()
